...
import { Plus, Key, Archive, Edit, Trash2, X, Check, Search, MoreHorizontal, Settings } from 'lucide-react';
import { 
  getPermissions, 
  getResources,
  createPermission, 
...
const getPermissionColor = (name: string) => {
  const colors = [
    'bg-amber-400', 'bg-orange-400', 'bg-red-400', 'bg-pink-400',
...
export default function PermissionsPage() {
  ...
  const [configPermission, setConfigPermission] = useState<Permission | null>(null);
  ...
  const openConfigModal = (permission: Permission) => {
    setConfigPermission(permission);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">加载权限数据...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-4xl font-display font-extrabold text-gray-900 mb-3 tracking-tight">权限管理</h1>
              <p className="text-lg font-medium text-gray-600 tracking-wide">管理系统权限和操作控制</p>
            </div>
            <button
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex items-center gap-4">
              {selectedPermissions.length > 0 && (
                <div className="flex items-center gap-3">
                  <span className="text-base font-medium text-gray-700">
                    已选择 <span className="font-bold text-blue-600">{selectedPermissions.length}</span> 个权限
                  </span>
              ...
              <div className="flex items-center gap-2 text-base font-medium text-gray-700">
                ...
                <span className="font-bold text-gray-900">{filteredPermissions.length}</span>
                <span>个权限</span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          {filteredPermissions.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3 tracking-tight">暂无权限</h3>
              <p className="text-base font-medium text-gray-600 mb-6">创建第一个权限开始管理系统操作</p>
              <button
                onClick={() => setShowCreateModal(true)}
            ...
            <>
              ...
              <div className="divide-y divide-gray-100/60">
                {filteredPermissions.map((permission, index) => (
                  <div key={permission.id} className="group hover:bg-gradient-to-r hover:from-blue-50/40 hover:to-indigo-50/20 transition-all duration-300 ease-out border-l-4 border-transparent hover:border-blue-400/30">
                    {/* 桌面端布局 */}
                    <div className="hidden lg:flex w-full px-6 py-5 items-center">
                      ...
                      <div className="flex-1 min-w-0 px-4">
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0">
                            <div className={`w-10 h-10 ${getPermissionColor(permission.name)} rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm border-2 border-white group-hover:shadow-md transition-all duration-200`}>
                      ...
                      <div className="flex-1 min-w-0 px-4">
                        <div className="flex flex-wrap gap-1 ml-2">
                          <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-bold bg-orange-50 text-orange-800 border border-orange-200/60">
                          ...
                          {openDropdown === permission.id && (
                            <>
                              ...
                              <div className="absolute right-0 top-full mt-2 w-48 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/60 py-2 z-20 ring-1 ring-black/5">
                                <button
                                  onClick={() => openConfigModal(permission)}
                    ...
                    <div className="lg:hidden p-6 space-y-4">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                          <div className="flex-shrink-0">
                            <div className={`w-10 h-10 ${getPermissionColor(permission.name)} rounded-xl flex items-center justify-center text-white font-semibold`}>
                              <Key className="w-5 h-5" />
                            </div>
                          </div>
                          <div className="min-w-0 flex-1">
                      ...
                      <div>
                        <p className="text-xs text-gray-500 mb-2">适用资源</p>
              ...
              >
                ...
                <div className="relative p-8">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h3 className="text-3xl font-display font-extrabold text-gray-900 mb-2 tracking-tight">创建新权限</h3>
                      <p className="text-base font-medium text-gray-600">添加新的权限到系统中</p>
                    </div>
                    <button
                      onClick={() => setShowCreateModal(false)}
                    ...
                    <div className="space-y-5">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="例如: resource:action"
                      ...
                      <div>
                        <label className="block text-base font-bold text-gray-800 mb-3">
                          权限描述 <span className="text-gray-500 font-medium">(可选)</span>
                        </label>
                        <textarea
                          value={permDesc}
      ...
      {/* 权限配置弹窗 */}
      {showConfigModal && configPermission && (
        <>
          <div 
            className="fixed inset-0 bg-black/20 backdrop-blur-md z-40 transition-all duration-300 ease-out"
                ...
                <div className="relative p-8">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center space-x-4">
                      <div className={`w-14 h-14 ${getPermissionColor(configPermission.name)} rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg ring-4 ring-white`}>
                        <Key className="w-8 h-8" />
                      </div>
                      <div>
                        <h3 className="text-3xl font-display font-extrabold text-gray-900 tracking-tight">{configPermission.name}</h3>
                        <p className="text-base font-semibold text-gray-700">{configPermission.description || '无描述'}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => setShowConfigModal(false)}
                    ...
                    <div>
                      ...
                      <div className="bg-white/70 backdrop-blur-sm border border-gray-200/60 rounded-2xl p-6">
                        <div className="text-center">
                          ...
                          <h5 className="text-lg font-bold text-gray-800 mb-2">适用于所有资源</h5>
                          <p className="text-base font-medium text-gray-600">此权限适用于系统中的所有资源类型</p>
                        </div>
                      </div>
                    </div>
                  </div>
